...
{%- endmacro -%}
{########################################################################################}
{%- macro render_kernel_launcher_debug_output(prefix,stage,kernel_name,global_vars,global_reduced_vars) -%}
...
{{ render_global_param_decls(kernel.global_vars,"","",",\n") | indent(4,True) }}
){
{{ render_local_var_decls(kernel.local_vars) | indent(2,True) }}
{{ render_shared_var_decls(kernel.shared_vars) | indent(2,True) }}
{{kernel.c_body | indent(2, True)}}
...
{{ render_kernel_launcher_debug_output(kernel.name+":gpu:in:","INPUT",kernel.name,kernel.global_vars,kernel.global_reduced_vars) | indent(2,True) }}
{% endif %}
...
{% if kernel_launcher.debug_output %}
{{ render_kernel_launcher_debug_output(kernel.name+":gpu:out:","OUTPUT",kernel.name,kernel.global_vars,kernel.global_reduced_vars) | indent(2,True) }}
{% endif %}
  return hipSuccess;
}
{%- endmacro -%}
...
extern "C" hipError_t {{kernel.launcher_name}}_f(
{{ render_all_global_param_decls(kernel.global_vars,kernel.global_reduced_vars) | indent(4,True) }}
);
{%- endmacro -%}
{########################################################################################}
{%- macro render_cpu_kernel_launcher(kernel,kernel_launcher) -%}
{% set num_global_vars = kernel.global_vars|length + kernel.global_reduced_vars|length %}
extern "C" hipError_t {{kernel_launcher.name}}(
    const int& sharedmem,
...
{% if kernel_launcher.debug_output %}
{{ render_kernel_launcher_debug_output(kernel.name+":cpu:in:","INPUT",kernel.name,kernel.global_vars,kernel.global_reduced_vars) | indent(2,True) }}
{% endif %}
  {{kernel_launcher.name}}_f({{render_all_global_params(kernel.global_vars,kernel.global_reduced_vars,False)}});
{% if kernel_launcher.debug_output %}
{{ render_kernel_launcher_debug_output(kernel.name+":cpu:out:","OUTPUT",kernel.name,kernel.global_vars,kernel.global_reduced_vars) | indent(2,True) }}
{% endif %}
  return hipSuccess;
}
{%- endmacro -%}
