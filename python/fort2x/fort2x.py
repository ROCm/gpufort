...
class CodeGenerator:
    ...
    def __init__(self,
        ...
        * *default_modules* (`list`):
            Default modules to use by any interface or modified Fortran module, program, or procedure. [default: fort2x.DEFAULT_MODULES].
        * *default_includes* (`list`):
            Default includes for generated C++ files. [default: fort2x.DEFAULT_INCLUDES].
        """
        self.stree = stree  
        self.index = index
        ...
        #
        self.cpp_filegen             = fort2x.filegen.CppFileGenerator(prolog=self.cpp_file_preamble)
    ...
    def _device_procedure_filter(self,
        ...
        return isinstance(stprocedure, scanner.STProcedure) and\
    ...
    def _modify_stcontainer(self,
        ...
        indent = stcontainer.first_line_indent()+" "*2
        ...
        if not stcontainer.has_contains_statement():
            stend.add_to_prolog(CodeGenerator._indent("contains",indent),prepend=True)   
    def _traverse_scanner_tree(self):
        ...
        def traverse_node_(stnode):
            ...
            if isinstance(stnode,scanner.STRoot):
                ...
                if isinstance(stnode.parent, scanner.STRoot):
                    inode = next((irecord for irecord in self.index if irecord["name"] == stnode_name),None)
                    cpp_file_name = "{}{}".format(CodeGenerator._fort2x_node_name(stnode),
                                                  self.cpp_file_ext)
                    cpp_filegen = fort2x.filegen.CppFileGenerator(guard=CodeGenerator._create_cpp_guard(cpp_file_name),
                                                                  prolog=self.cpp_file_preamble,
                                                                  default_includes=self.default_includes)
                    fortran_modulegen = fort2x.filegen.FortranModuleGenerator(name=inode["name"],
                                                                              default_modules=self._make_module_dicts(self.default_modules))
                else:
                    assert isinstance(stnode,scanner.STProcedure)
                    inode = stnode.index_record    
                    if isinstance(stnode.parent,scanner.STModule):
                        fortran_modulegen = fort2x.filegen.FortranModuleGenerator(name=inode["name"],
                                                                                  default_modules=self._make_module_dicts(self.default_modules))
                if inode == None:
                ...
                # types
                itypes = inode["types"]
                if len(itypes) and isinstance(stnode.parent,(scanner.STRoot,scanner.STModule)):
                ...
                # finalize
                if isinstance(stnode.parent,(scanner.STRoot,scanner.STModule)) and\
