...
{%- macro render_launcher_binding(kernel,
                                  launcher) -%}
{%- set all_global_vars = kernel.global_vars+kernel.global_reduced_vars -%}
{%- set num_all_global_vars = kernel.global_vars|length + kernel.global_reduced_vars|length -%}
function {{launcher.name}}(&
{% if launcher.kind in ["hip"] %}
    grid, block,&
{% endif %}
    sharedmem,stream{{"," if num_all_global_vars > 0 }}&
{{cm.render_params(all_global_vars,sep=",&\n") | indent(4,True)}}) bind(c,name="{{launcher.name}}_") &
      result(ierr)
{% if launcher.used_modules|length %}
{{ cm.render_used_modules(launcher.used_modules) | indent(2,True) -}}
{% endif %}
{% if num_all_global_vars > 0 %}
{{ cm.render_imports(all_global_vars) | indent(2,True) -}}
{% endif %}
  implicit none
{% if launcher.kind in ["hip"] %}  
...
{% endif %}
  ...
  type(c_ptr),intent(in)    :: stream{{"," if num_all_global_vars > 0}}
{{ cm.render_param_decls(all_global_vars) | indent(2,True) }}
