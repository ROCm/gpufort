...
class CodeGenerator(abc.ABC):
    ...
    def __init__(self,
        ...
        * *fortran_module_suffix* (`str`):
        ...
        self.stree = stree  
        self.index = index
        util.kwargs.set_from_kwargs(self,"kernels_to_convert",["*"],**kwargs)
        util.kwargs.set_from_kwargs(self,"cpp_file_preamble",opts.cpp_file_preamble,**kwargs)
        util.kwargs.set_from_kwargs(self,"cpp_file_ext",opts.cpp_file_ext,**kwargs)
        util.kwargs.set_from_kwargs(self,"fortran_module_suffix",opts.fortran_module_suffix,**kwargs)
        util.kwargs.set_from_kwargs(self,"default_modules",opts.fortran_default_modules,**kwargs)
        util.kwargs.set_from_kwargs(self,"default_includes",opts.cpp_default_includes,**kwargs)
        # adjusted by subclasses
        #
        self.cpp_filegen             = filegen.CppFileGenerator(prolog=self.cpp_file_preamble)
    ...
    def _loop_kernel_filter(self,
                             child):
        return isinstance(child, scanner.tree.STLoopNest) and self._consider_kernel(child)
    ...
    def _modify_stcontainer(self,
        ...
        if len(fortran_modulegen.rendered_types):
            for mod in self.default_modules:
                stcontainer.add_to_epilog("{}use {}".format(indent,mod),prepend=True)
        # types and interfaces
        stlastdeclnode = stcontainer.last_entry_in_decl_list()
        for snippet in fortran_modulegen.rendered_types:
            stlastdeclnode.add_to_epilog(CodeGenerator._indent(snippet,indent))
        ...
        if not stcontainer.has_contains_statement():
            stend.add_to_prolog(CodeGenerator._indent("contains",indent_parent),prepend=True)   
        ...
        def traverse_node_(stnode):
            ...
            if isinstance(stnode,scanner.tree.STRoot):
                ...
                self._render_device_procedure(stnode,
                           ...
                           (scanner.tree.STProgram,
                           scanner.tree.STModule,
                ...
                if isinstance(stnode.parent, scanner.tree.STRoot):
                    ...
                    cpp_file_name = "{}{}".format(CodeGenerator._fort2x_node_name(stnode),
                                                  self.cpp_file_ext)
                    cpp_filegen = filegen.CppFileGenerator(guard=CodeGenerator._create_cpp_guard(cpp_file_name),
                                                                  prolog=self.cpp_file_preamble,
                                                                  default_includes=self.default_includes)
                    fortran_modulegen = filegen.FortranModuleGenerator(name=inode["name"]+opts.fortran_module_suffix,
                                                                       default_modules=self._make_module_dicts(self.default_modules))
                else:
                ...
                # types
                itypes = inode["types"]
                if len(itypes) and isinstance(stnode.parent,(scanner.tree.STRoot,scanner.tree.STModule)):
