...
"""
        ...
        util.kwargs.set_from_kwargs(self,"emit_debug_code",opts.emit_debug_code,**kwargs)
        util.kwargs.set_from_kwargs(self,"emit_gpu_kernel",opts.emit_gpu_kernel,**kwargs)
        util.kwargs.set_from_kwargs(self,"emit_grid_launcher",opts.emit_grid_launcher,**kwargs)
        util.kwargs.set_from_kwargs(self,"emit_problem_size_launcher",opts.emit_problem_size_launcher,**kwargs)
        util.kwargs.set_from_kwargs(self,"emit_cpu_launcher",opts.emit_cpu_launcher,**kwargs)
        util.kwargs.set_from_kwargs(self,"emit_fortran_interfaces",opts.emit_fortran_interfaces,**kwargs)
    ...
    def __render_kernel(self,
        ...
        if self.emit_gpu_kernel:
            cpp_filegen.rendered_kernels += (mykernelgen.render_begin_kernel_comment_cpp()
                                            + mykernelgen.render_gpu_kernel_cpp()
                                            + mykernelgen.render_end_kernel_comment_cpp())
        rendered_launchers_cpp  = []
        rendered_interfaces_f03 = []
        grid_launcher = mykernelgen.create_launcher_context("hip",
        ...
        if is_loopnest and self.emit_problem_size_launcher:
            ...
            rendered_launchers_cpp  += mykernelgen.render_gpu_launcher_cpp(problem_size_launcher)
            rendered_interfaces_f03 += mykernelgen.render_launcher_interface_f03(problem_size_launcher)

        if is_loopnest and self.emit_cpu_launcher:
            cpu_launcher = copy.deepcopy(grid_launcher)
            cpu_launcher["kind"] = "cpu"
            rendered_launchers_cpp  += mykernelgen.render_cpu_launcher_cpp(cpu_launcher)
            rendered_interfaces_f03 += mykernelgen.render_launcher_interface_f03(cpu_launcher)
            fortran_filegen.rendered_routines += (mykernelgen.render_begin_kernel_comment_f03()
                                                 + mykernelgen.render_cpu_routine_f03(cpu_launcher)
                                                 + mykernelgen.render_end_kernel_comment_f03())
        if len(rendered_launchers_cpp):
            cpp_filegen.rendered_launchers += (mykernelgen.render_begin_kernel_comment_cpp()
                                              + rendered_launchers_cpp
                                              + mykernelgen.render_end_kernel_comment_cpp())
        if self.emit_fortran_interfaces and len(rendered_interfaces_f03):
            fortran_filegen.rendered_interfaces += (mykernelgen.render_begin_kernel_comment_f03()
                                                   + rendered_interfaces_f03
                                                   + mykernelgen.render_end_kernel_comment_f03())
    def _render_loop_nest(self,
        ...
        scope = scope.create_scope(self.index,
                                   stloopnest._parent.tag())
        
        mykernelgen = hipkernelgen.HipKernelGenerator4LoopNest(stloopnest.kernel_name(),
    ...
    def _render_device_procedure(stprocedure,
        ...
        scope = scope.create_scope(index,
                                    stprocedure._parent.tag())
        if stprocedure.is_kernel_subroutine():  
            mykernelgen = hipkernelgen.HipKernelGenerator4CufKernel(iprocedure["name"],
            ...
            mykernelgen = hipkernelgen.HipKernelGenerator4AcceleratorRoutine(stprocedure.kernel_name(),
                                                                                       stprocedure.kernel_hash(),
                                                                                       stprocedure.parse_result,
                                                                                       iprocedure,
                                                                                       scope,
