...
class Node():
    ...
    def statements(self,linemaps,end):
        ...
        return [stmt["body"] for stmt in linemapper.get_linemaps_content(linemaps_of_node,"statements",self._begin[1],end[1])]
    __repr__ = __str__
...
def _intrnl_parse_statements(linemaps,filepath):
    ...
    def ParseDeclarationTask_(parent_node,input_text):
        ...
        try:
            ...
            util.logging.log_exception(opts.log_prefix,"_intrnl_parse_statements.ParseDeclarationTask_","failed: "+str(e))
            sys.exit(2)
        access_lock.acquire()
    ...
    class ParseAccDeclareJob_:
        ...
        def run(self):
            ...
            log_enter_job_or_task_(self._parent_node, msg)
            #
            parse_result = translator.tree.grammar.acc_declare.parseString(self._input_text)[0]
            for var_context in self._parent_node._data["variables"]:
    ...
    def Use(tokens):
        ...
        if current_node._kind != "root":
            ...
            for pair in tokens[2]:
                ...
                renamed = original if pair[1] is None else translator.tree.make_f_str(pair[1])
    ...
    def AccRoutine():
        ...
        if current_node != root:
            parse_result = translator.tree.grammar.acc_routine.parseString(current_statement)[0]
    ...
    def try_to_parse_string(expression_name,expression):
        try:
           ...
           util.logging.log_debug3(opts.log_prefix,"_intrnl_parse_statements","did not find expression '{}' in statement '{}'".format(expression_name,current_statement))
           util.logging.log_debug4(opts.log_prefix,"_intrnl_parse_statements",str(e))
           return False
        ...
        if current_linemap["is_active"]:
            for current_statement_no,stmt in enumerate(current_linemap["statements"]):
                ...
                if not consider_statement(current_statement):
                    util.logging.log_debug3(opts.log_prefix,"_intrnl_collect_statements","ignore statement '{}'".format(current_statement))
                else:
                    util.logging.log_debug3(opts.log_prefix,"_intrnl_parse_statements","process statement '{}'".format(current_statement))
    ...
    if num_post_parsing_jobs > 0:
        ...
        util.logging.log_debug(opts.log_prefix,"_intrnl_parse_statements","apply variable modifications --- done") 
...
def update_index_from_snippet(index,snippet,preproc_options=""):
    ...
    linemaps    = linemapper.preprocess_and_normalize(snippet.splitlines(),"dummy.f90",macro_stack)
...
def search_derived_types(imodule,search_subprograms=True):
    ...
    util.logging.log_leave_function(opts.log_prefix,"_intrnl_search_derived_types",
