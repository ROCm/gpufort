...
pp_number = pyparsing.pyparsing_common.number.copy().setParseAction(
    lambda tk: str(tk[0]))
pp_bool_true = pyparsing.Regex(r"\.true\.|true|1",
                               re.IGNORECASE).setParseAction(lambda tk: "1")
pp_bool_false = pyparsing.Regex(r"\.false\.|false|0",
                                re.IGNORECASE).setParseAction(lambda tk: "0")
pp_defined = pyparsing.Regex(r"defined\(\s*(?P<name>\w+)\s*\)", re.IGNORECASE)

pp_value = pyparsing.Forward()

pp_op_and = pyparsing.Regex(r"(\.and\.)|(&&)",
                            re.IGNORECASE).setParseAction(lambda tk: " and ")
pp_op_or = pyparsing.Regex(r"(\.or\.)|(\|\|)",
                           re.IGNORECASE).setParseAction(lambda tk: " or ")
pp_op_not = pyparsing.Regex(r"(\.not\.)|!",
                            re.IGNORECASE).setParseAction(lambda tk: "not ")
...
pp_arithm_logic_expr = pyparsing.infixNotation(
    pp_value, [(pp_op_not, 1, pyparsing.opAssoc.RIGHT),
...
pp_macro_eval = pp_ident + pyparsing.Group(
    pyparsing.Optional(arg_list, default=[])
) # ! setResultName causes issue when args are macro evals again
pp_value <<= (pp_number | pp_bool_true | pp_bool_false | pp_defined
              | pp_macro_eval) # | pp_char ) # ! defined must come before eval
...
# include
pp_dir_include = pyparsing.Regex(
    r"#\s*include\s+\"(?P<filename>[" + pyparsing.printables + r"]+)\"",
    re.IGNORECASE)
# define / undef
pp_dir_define = pyparsing.Regex(
    r"#\s*define\s+(?P<name>\w+)(\((?P<args>(\w|[, ])+)\))?\s+(?P<subst>.*)",
    re.IGNORECASE)
