    ...
    try:
        ...
        if stripped_first_line.startswith("if"):
            ...
            if region_stack1[-1] and stripped_first_line.startswith("ifdef"):
                ...
                result = grammar.pp_dir_ifndef.parseString(single_line_statement,parseAll=True)
                condition = "!defined("+result.name+")"
            elif region_stack1[-1]: # and stripped_first_line.startswith("if"):
                result    = grammar.pp_dir_if.parseString(single_line_statement,parseAll=True)
...
def _group_modified_linemaps(linemaps,wrap_in_ifdef):
    ...
    for i,linemap in enumerate(linemaps):
        ...
        elif opts.line_grouping_include_blank_lines and len(current_linemaps) and contains_blank_line_(linemap) and borders_previous_linemap_(linemap):
            current_linemaps.append(linemap)
    # append last block
    append_current_block_if_non_empty_()
    return blocks
...
def preprocess_and_normalize(fortran_file_lines,fortran_file_path,macro_stack=[],region_stack1=[True],region_stack2=[True]):
    ...
    util.logging.log_leave_function(opts.log_prefix,"preprocess_and_normalize")
    return linemaps
...
def modify_file(linemaps,**kwargs):
    ...
    * *ifdef_macro* (``str``):
    ...
    if not have_lines:
        file_content, have_content = util.kwargs.get_value("file_content",None,**kwargs)
        if have_content:
