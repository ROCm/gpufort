...
def _parse_file(linemaps,index):
    ...
    def ascend_():
        ...
        nonlocal current_linemap
        nonlocal current_statement_no
        assert not current_node.parent is None, "In file {}: parent of {} is none".format(current_file,type(current_node))
        current_node_id = current_node.kind
        if current_node.name != None:
            current_node_id += " '"+current_node.name+"'"
    ...
    def End():
        ...
        assert type(current_node) in [tree.STModule,tree.STProgram,tree.STProcedure], "In file {}: line {}: type is {}".format(current_file, current_statement_no, type(current_node))
        if type(current_node) is tree.STProcedure and current_node.must_be_available_on_device():
    ...
    def CufLibCall(tokens):
        ...
        if not type(current_node) in [cuf.STCufLibCall,cuf.STCufKernelCall]:
            ...
            assert type(new.parent) in [tree.STModule,tree.STProcedure,tree.STProgram], type(new.parent)
            append_if_not_recording_(new)
    ...
    datatype_reg = pyparsing.Regex(r"\s*\b(type\s*\(\s*\w+\s*\)|character|integer|logical|real|complex|double\s+precision)\b") 
    ...
    def scan_string(expression_name,expression):
        ...
        if matched:
           util.logging.log_debug3(opts.log_prefix,"parse_file.scanString","found expression '{}' in line {}: '{}'".format(expression_name,current_linemap["lineno"],current_linemap["lines"][0].rstrip()))
        else:
           util.logging.log_debug4(opts.log_prefix,"parse_file.scanString","did not find expression '{}' in line {}: '{}'".format(expression_name,current_linemap["lineno"],current_linemap["lines"][0].rstrip()))
        return matched
    ...
    def try_to_parse_string(expression_name,expression,parseAll=False):
        ...
        try:
           ...
           util.logging.log_debug3(opts.log_prefix,"parse_file.try_to_parse_string","found expression '{}' in line {}: '{}'".format(expression_name,current_linemap["lineno"],current_linemap["lines"][0].rstrip()))
           return True
        except pyparsing.ParseBaseException as e: 
           util.logging.log_debug4(opts.log_prefix,"parse_file.try_to_parse_string","did not find expression '{}' in line '{}'".format(expression_name,current_linemap["lines"][0]))
           util.logging.log_debug5(opts.log_prefix,"parse_file.try_to_parse_string",str(e))
           return False
    ...
    for current_linemap in linemaps:
        ...
        if condition1 and condition2:
            for current_statement_no,current_statement in enumerate(current_linemap["statements"]):
                ...
                current_tokens                       = util.parsing.tokenize(current_statement["body"].lower(),padded_size=6)
                ...
                if len(current_tokens):
                    ...
                    if not keep_recording:
                        ...
                        if "allocate" in current_tokens:
                            try_to_parse_string("allocate",tree.grammar.ALLOCATE) 
                        if "function" in current_tokens:
                            try_to_parse_string("function",tree.grammar.function_start)
                        if "subroutine" in current_tokens:
                            try_to_parse_string("subroutine",tree.grammar.subroutine_start)
                        # 
                        if current_tokens[0] == "use":
                            ...
                            try_to_parse_string("module",tree.grammar.module_start)
                        elif current_tokens[0:2] == ["end","type"]:
                            PlaceHolder()
                        elif current_tokens[0] == "program":
                            try_to_parse_string("program",tree.grammar.program_start)
...
def postprocess(stree,index,hip_module_suffix):
    ...
    math libraries.
    """
    if "hip" in opts.destination_dialect or len(opts.kernels_to_convert_to_hip):
            ...
            for kernel in kernels:
                if "hip" in opts.destination_dialect or\
                  ...
                  kernel.kernel_name() in kernels_to_convert_to_hip:
                    stnode = kernel.parent.find_first(filter=lambda child: isinstance(child,(tree.STUseStatement,
                                                                                             tree.STDeclaration,
                                                                                             tree.STContains,
                                                                                             tree.STPlaceHolder)))
...
def parse_file(**kwargs):
    ...
    * *index* (``list``):
    ...
    file_is_indexed,_ = util.kwargs.get_value("file_is_indexed",have_index,**kwargs)
    # handle main file
    file_content, have_file_content = util.kwargs.get_value("file_content",None,**kwargs)
    linemaps, have_linemaps         = util.kwargs.get_value("file_linemaps",None,**kwargs)
    file_path,have_file_path        = util.kwargs.get_value("file_path","<unknown>",**kwargs)
    ...
    if not have_linemaps:
       linemaps = linemapper.read_lines(file_content.splitlines(keepends=True),
                                        preproc_options,
                                        file_path)
    ...
    other_files_contents,have_other_files_contents = util.kwargs.get_value("other_files_contents",[],**kwargs)
    other_files_paths,have_other_files_paths       = util.kwargs.get_value("other_files_paths",[],**kwargs)
    ...
    for content in other_files_contents:
        indexer.update_index_from_snippet(index,
                                          content,
                                          preproc_options)
