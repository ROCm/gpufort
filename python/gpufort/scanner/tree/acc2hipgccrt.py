...
class Acc2HipGccRT(acc.AccBackendBase):
    ...
    def _handle_if(self,parse_result):
        ...
        if clause != None:
            ...
            return ""
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[],handle_if=False):
...
class AccLoopNest2HipGccRT(Acc2HipGccRT):
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]):
        ...
        if stnode.is_parallel_loop_directive() or stnode.is_kernels_loop_directive():
            partial_result, _ = Acc2HipGccRT.transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index,handle_if=False) 
            result = partial_result
        partial_result, _ = base.STLoopNest.transform(stnode,joined_lines,joined_statements,statements_fully_cover_lines,index) 
        result += partial_result
...
acc.register_acc_backend("hip-gcc-rt",
                         Acc2HipGccRT,
                         AccLoopNest2HipGccRT,
