...
class Acc2HipGpufortRT(acc.AccBackendBase):
    # clauses
...
class AccLoopNest2HipGpufortRT(Acc2HipGpufortRT):
        ...
        if len(default_clauses):
            ...
            if value == "present":
                for var_expr in unmapped_arrays:
                    ...
                    util.logging.log_warning(LOG_PREFIX,"AccLoopNest2HipGpufortRT._handle_default","'default(none)' specified but no map for the following variables found: ".format(", ".join(unmapped_arrays)))
        else: # default strategy: present_or_copy
            for var_expr in unmapped_arrays:
                deviceptr = dev_var_name(var_expr)
                if not deviceptr in temp_vars: 
                    temp_vars.append(deviceptr)
                    result += template.format(indent=indent,var=var_expr,asyncr="",alloc=",or=gpufort_acc_event_copy",dev_var=deviceptr)
    ...
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]):
        ...
        if transformed:
            ...
            result = "\n".join([result.rstrip("\n"), partial_result])
        partial_result, _ = base.STLoopNest.transform(stloopkernel,joined_lines,joined_statements,statements_fully_cover_lines,index) 
        result = "\n".join([result.rstrip("\n"), partial_result])
...
class Acc2HipGpufortRTPostprocess(acc.AccPostprocessBackendBase):
            ...
            for ivar in scope_vars:
                ...
                if not is_allocatable:
                    ...
                    if is_pointer:
                        ...
                        acc_present_template = (cond+HIP_GPUFORT_RT_ACC_PRESENT.replace("{indent}",""))
                    else:
...
acc.register_acc_backend("hip-gpufort-rt",
                          Acc2HipGpufortRT,
