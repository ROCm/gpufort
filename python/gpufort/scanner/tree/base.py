...
class STContainerBase(STNode): 
    ...
    def local_and_dummy_variable_names(self,index):
        ...
        scope      = scope.create_scope(index,self.tag())
        ...
        if type(self) == STProgram:
            ...
            irecord, _ = scope.search_index_for_subprogram(index,parent_tag,self.name)
        ...
        for lib in HOST_MODULES:
            ...
            if lib_lower in snippet.lower():
                ...
                if lib_lower == "cudafor":
                    snippet = re.sub(r"\bcudafor\b",opts.hip_module_name,snippet,re.opts.IGNORECASE)
                    snippet = snippet.rstrip("\n")+"\n" + "{0}use hipfort_check".format(indent) 
                snippet = re.sub(r"\bcu(\w+)\b",opts.hip_math_module_prefix+r"hip\1",snippet,re.opts.IGNORECASE)
...
class STProcedure(STContainerBase):
    def __init__(self,name,parent_tag,kind,first_linemap,first_linemap_first_statement,index):
        ...
        self.code = []
        # check attributes
        self.index_record, _ = scope.search_index_for_subprogram(index,parent_tag,name)
...
class STDeclaration(STNode):
    ...
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]):
        ...
        for var_name in self._vars:
             ivar,discovered = scope.search_index_for_variable(\
...
class STNonZeroCheck(STNode):
    ...
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]): # TODO
        ...
        for tokens,start,end in translator.non_zero_check.scanString(result):
            ...
            ivar,_  = scope.search_index_for_variable(index,self.parent.tag(),\
              lhs_name)
...
class STAllocated(STNode):
    ...
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]): # TODO
        ...
        result, transformed = util.pyparsing.replace_all(joined_statements,translator.allocated,repl)
        assert result != None
        return result, transformed
...
class STAllocate(STNode):
    def __init__(self,first_linemap,first_linemap_first_statement):
        STNode.__init__(self,first_linemap,first_linemap_first_statement)
        self.parse_result   = translator.allocate.parseString(self.statements()[0])[0]
        self.variable_names = self.parse_result.variable_names() 
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]): # TODO
        indent = self.first_line_indent()
        result1, transformed1 = handle_allocate_cuf(self,joined_statements,index) 
        result2, transformed2 = handle_allocate_acc(self,result1,index)
        return result2, (transformed1 or transformed2)
             
class STDeallocate(STNode):
    def __init__(self,first_linemap,first_linemap_first_statement):
        STNode.__init__(self,first_linemap,first_linemap_first_statement)
        self.parse_result   = translator.deallocate.parseString(self.statements()[0])[0]
        self.variable_names  = self.parse_result.variable_names() 
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]): # TODO
        """
        ...
        """
        result1, transformed1 = handle_deallocate_cuf(self,joined_statements,index) 
        result2, transformed2 = handle_deallocate_acc(self,result1,index)
        return result2, (transformed1 or transformed2)
...
class STMemcpy(STNode):
    def __init__(self,first_linemap,first_linemap_first_statement):
        STNode.__init__(self,first_linemap,first_linemap_first_statement)
        self._parse_result = translator.memcpy.parseString(self.statements()[0])[0]
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]): # TODO
        def repl_memcpy_(parse_result):
            dest_name = parse_result.dest_name_f_str()
            src_name  = parse_result.src_name_f_str()
            dest_indexed_var,_ = scope.search_index_for_variable(index,self.parent.tag(),\
              dest_name,error_handling="off")
            src_indexed_var,_  = scope.search_index_for_variable(index,self.parent.tag(),\
        ...
        #return util.pyparsing.replace_all(joined_statements,translator.memcpy,repl_memcpy)
    ...
    def transform(self,joined_lines,joined_statements,statements_fully_cover_lines,index=[]):
        ...
        if not opts.keep_cuda_lib_names:
            def repl_memcpy(parse_result):
                ...
                subst = parse_result.hip_f_str(dest_on_device,src_on_device)
                return (subst, True)
            snippet,_ = util.pyparsing.replace_all(snippet,translator.cuf_cudamemcpy_variant,repl_memcpy)
        def repl_cublas(parse_result):
            subst = parse_result.f_str(indent)
            return (subst, True)
        snippet,have_cublas = util.pyparsing.replace_all(snippet,translator.cuf_cublas_call,repl_cublas)
        if have_cublas:
        ...
        for tokens,start,end in translator.cuf_kernel_call.scanString(snippet):
            ...
            for ttexpr in parse_result._args:
                ...
                for rvalue in translator.find_all(ttexpr,translator.TTRValue):
                    ...
                    ivar, discovered = scope.search_index_for_variable(index,self.parent.tag(),\
                       rvalue.name())
                    if discovered:
