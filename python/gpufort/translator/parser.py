...
def _intrnl_parse_fortran_code(statements,scope=None):
    ...
    def append_(node,kind=None):
        ...
        if kind != None:
            util.logging.log_debug2(opts.log_prefix,"_intrnl_parse_fortran_code.append_","found {} in statement '{}'".format(kind,stmt))
    def descend_(node,kind,inc_level=True):
        ...
        if inc_level:
            level += 1
        util.logging.log_debug2(opts.log_prefix,"_intrnl_parse_fortran_code.append_","enter {} in statement '{}'".format(kind,stmt))
    def ascend_(kind):
        nonlocal curr
        nonlocal stmt
        nonlocal level
        curr  = curr.parent
        level = min(level-1,0)
        util.logging.log_debug2(opts.log_prefix,"_intrnl_parse_fortran_code.append_","leave {} in statement '{}'".format(kind,stmt))
    
    # error handling
    def error_(expr,exception=None):
        nonlocal stmt1
        util.logging.log_error(opts.log_prefix,"_intrnl_parse_fortran_code","failed to parse {} expression '{}'".format(expr,stmt))
        if exception != None:
            debug_msg = ": "+str(exception)
            util.logging.log_debug(opts.log_prefix,"_intrnl_parse_fortran_code",debug_msg)
        sys.exit(2) # TODO error code
    def warn_(expr,exception=None):
        nonlocal stmt1
        util.logging.log_warn(opts.log_prefix,"_intrnl_parse_fortran_code","ignored {} expression '{}'".format(expr,stmt))
        if exception != None:
            util.logging.log_debug(opts.log_prefix,"_intrnl_parse_fortran_code",str(exception))
        sys.exit(2) # TODO error code
    def ignore_(expr):
        nonlocal stmt1
        util.logging.log_debug3(opts.log_prefix,"_intrnl_parse_fortran_code","ignored {} '{}'".format(expr,stmt))
    # parser loop
    ttree               = tree.TTRoot()
    curr                = ttree
    ...
    for stmt1 in statements:
        ...
        util.logging.log_debug3(opts.log_prefix,"_intrnl_parse_fortran_code","process statement '{}' (preprocessed: '{}')".format(stmt1,stmt))
        if len(tokens):
            util.logging.log_debug4(opts.log_prefix,"_intrnl_parse_fortran_code","tokens=['{}']".format("','".join(tokens)))
        # tree construction
        if util.parsing.is_blank_line(stmt):
            ...
            try:
                if util.parsing.is_ignored_fortran_directive(tokens):
                    ignore_("directive")
                elif util.parsing.is_fortran_offload_region_plus_loop_directive(tokens): # most complex first
                    parse_result = tree.grammar.loop_annotation.parseString(stmt,parseAll=True)
                    curr_offload_region = parse_result[0]
                    curr_offload_loop   = parse_result[0] 
                    util.logging.log_debug2(opts.log_prefix,"_intrnl_parse_fortran_code.append_","found {} in statement '{}'".format("loop offloading directive",stmt))
                elif util.parsing.is_fortran_offload_region_directive(tokens):
                    parse_result = tree.grammar.parallel_region_start.parseString(stmt,parseAll=True)
                    curr_offload_region = parse_result[0]
                    util.logging.log_debug2(opts.log_prefix,"_intrnl_parse_fortran_code.append_","found {} in statement '{}'".format("begin of offloaded region",stmt))
                elif util.parsing.is_fortran_offload_loop_directive(tokens):
                    parse_result = tree.grammar.loop_annotation.parseString(stmt,parseAll=True)
                    curr_offload_loop = parse_result[0] 
                    util.logging.log_debug2(opts.log_prefix,"_intrnl_parse_fortran_code.append_","found {} in statement '{}'".format("loop directive",stmt))
                else:
                    warn_("directive",e)
        ...
        # do/while
            ...
            try: 
                parse_result = tree.grammar.fortran_do_while.parseString(stmt_no_comment,parseAll=True)
                descend_(tree.TTDoWhile(stmt,0,parse_result.asList()+[[]]),"do-while loop")
            except Exception as e:
                error_("do-while loop",e)
        elif util.parsing.is_do(tokens):
            try: 
                parse_result   = tree.grammar.fortran_do.parseString(stmt_no_comment,parseAll=True)
        ...
        # if-then-else
            ...
            try: 
                ...
                parse_result = tree.grammar.fortran_if_else_if.parseString(stmt_no_comment,parseAll=True)
                descend_(tree.TTIfElseIf(stmt_no_comment,0,parse_result.asList()+[[]]),"if branch")
            except Exception as e:
                error_("if",e)
            ...
            try: 
                parse_result = tree.grammar.fortran_if_else_if.parseString(stmt_no_comment,parseAll=True)
                descend_(tree.TTIfElseIf(stmt_no_comment,0,parse_result.asList()+[[]]),"else-if branch")
            except Exception as e:
                error_("else-if",e)
        ...
        # select-case
            ...
            try: 
                parse_result = tree.grammar.fortran_select_case.parseString(stmt_no_comment,parseAll=True)
                descend_(tree.TTSelectCase(stmt_no_comment,0,parse_result.asList()+[[]]),"select-case")
            except Exception as e:
            ...
            if type(curr) is tree.TTCase:
                ascend_("case")
            try: 
                parse_result = tree.grammar.fortran_case.parseString(stmt_no_comment,parseAll=True)
                descend_(tree.TTCase(stmt_no_comment,0,parse_result.asList()+[[]]),"case")
            except Exception as e:
                error_("else-if",e)
        ...
        # single statements
            ...
            try: 
                parse_result = tree.grammar.fortran_assignment.parseString(stmt_no_comment,parseAll=True)
                append_(parse_result[0],"assignment")
            except Exception as e:
                error_("assignment",e)
        elif util.parsing.is_subroutine_call(tokens):
            try: 
                parse_result = tree.grammar.fortran_subroutine_call.parseString(stmt_no_comment,parseAll=True)
...
def parse_declaration(fortran_statement):
    orig_tokens             = util.parsing.tokenize(fortran_statement.lower(),padded_size=10)
    tokens                  = orig_tokens
    util.logging.log_debug2(opts.log_prefix,"parse_declaration","tokens="+str(tokens))
    ...
    if tokens[0] == "type":
        ...
        # ex: integer * 4
        # ex: integer * ( 4*2 )
        kind_tokens = util.parsing.next_tokens_till_open_bracket_is_closed(tokens[2:],open_brackets=0)
        ...
        # ex: integer ( kind = 4 )
        # ex: integer ( kind ( 4 ) )
        kind_tokens = util.parsing.next_tokens_till_open_bracket_is_closed(tokens[3:],open_brackets=1)
        kind = "".join(kind_tokens[:-1])
        idx_last_consumed_token = 3 + len(kind_tokens)-1
    elif tokens[1] == "(" :
        # ex: integer ( 4 )
        # ex: integer ( 4*2 )
        kind_tokens = util.parsing.next_tokens_till_open_bracket_is_closed(tokens[2:],open_brackets=1)
        kind = "".join(kind_tokens[:-1])
        idx_last_consumed_token = 2 + len(kind_tokens)-1
        ...
        util.logging.log_error(opts.log_prefix,"parse_declaration","could not parse datatype in variable declaration statement '{}'".format(fortran_statement))
        sys.exit(2)
    ...
    if tokens[0] == "," and DOUBLE_COLON in tokens:
        ...
        util.logging.log_error(opts.log_prefix,"parse_declaration","could not parse qualifier list in variable declaration statement '{}'".format(fortran_statement))
        sys.exit(2)
    ...
    util.logging.log_debug2(opts.log_prefix,"parse_declaration","type="+str(datatype)+",kind="+str(kind)+",qualifiers="+str(qualifiers_raw)+","+"variables="+str(variables_raw))
    # construct declaration tree node
    qualifiers = []
    for qualifier in qualifiers_raw:
        if qualifier.startswith("dimension"):
            try:
                qualifiers.append( tree.grammar.dimension_qualifier.parseString(qualifier,parseAll=True)[0] )
            except:
                util.logging.log_error(opts.log_prefix,"parse_declaration","could not parse dimension qualifier in variable declaration statement '{}'".format(fortran_statement))
                sys.exit(2)
        elif qualifier.startswith("intent"):
            # ex: intent ( inout )
            intent_tokens = util.parsing.tokenize(qualifier)
            try:
                qualifiers.append( tree.grammar.intent_qualifier.parseString(qualifier,parseAll=True)[0] )
            except:
                util.logging.log_error(opts.log_prefix,"parse_declaration","could not parse intent qualifier in variable declaration statement '{}'".format(fortran_statement))
                sys.exit(2) 
        else:
    ...
    for var in variables_raw:
        try:
            variables.append( tree.grammar.declared_variable.parseString(var,parseAll=True)[0] )
        except:
            util.logging.log_error(opts.log_prefix,"parse_declaration","could not parse variable declaration '{}' in statement '{}'".format(var,fortran_statement))
            sys.exit(2)
...
def create_index_records_from_declaration(ttdeclaration):
    ...
    for ttdeclaredvariable in ttdeclaration._rhs:
        ...
        # arrays
        ivar["rank"]                       = 0
        dimension_qualifier = tree.find_all(ttdeclaration.qualifiers,tree.TTDimensionQualifier)
