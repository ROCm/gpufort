...
class TTAccDirective(base.TTStatement):
                    ...
                    for other in named_device_types:
                        if other.lower() == device_type.lower():
                            return util.error.SyntaxError(
                                "device type '{}' specified more than once in ".format(
                                    device_type)
                                + "'device_type' clauses"
                            )
    ...
    def walk_clauses_device_type(self, device_type):
        ...
        def clause_applies_to_device_type_(device_type):
            ...
            return (
                # no device_type clause encountered yet
                not len(current_device_types)
    ...
    def _check_clauses(self):
        ...
        for i, clause in enumerate(self.clauses):
            if isinstance(clause, UnprocessedGenericMappingClause):
                print(clause.kind)
            assert isinstance(clause, TTAccClause), "clause "+str(i) + \
                " is not of type TTAccClause, is of type "+str(type(clause))
...
class TTAccKernels(TTAccComputeConstruct):
    ...
    def _contains_single_assignment_statement(self):
        if len(self.body) == 1:
            return isinstance(self.body[0], arithexpr.TTAssignment)
        return False
...
class TTAccLoop(TTAccConstruct):
    ...
    def next_n_loops_in_body(self,
        ...
        def statement_is_allowed_(ttstmt):
            if isinstance(ttstmt, (
                ...
                # todo: double check that valid for all injected statements
                transformations.TTInjectedStatement,
                fortran.TTfBlock,
        ...
        loop_cls = (fortran.TTDo)  # todo: think do_concurrent
            ...
            for ttstmt in self.body:
                if isinstance(ttstmt, loop_cls):
                    ...
                    if len(loop_list) == num_loops:
                        return
                    descend_(ttstmt)
                elif statement_is_allowed_(ttstmt):
                    if perfect_nesting:
                        raise util.error.LookupError(
                            "loop nesting disrupted by statement of type '{}'".format(
                                type(ttstmt))
                        )
                    else:
    ...
    def associated_loops(self, device_type):
        ...
        """
        (collapse_specified, force_specified,
         collapse_arg) = self.collapse(device_type)
...
class TTAccRoutine(TTAccDirective):
    ...
    def _is_legal_clause(self, clause):
        return isinstance(clause, (
            ...
            TTAccClauseVector,
            TTAccClauseSeq,
            # :todo: identifier (as in language being compiled) vs string (unmodified)
            TTAccClauseBind,
            TTAccClauseDeviceType,
...
def acc_directive_parse_action(tokens):
    ...
    if isinstance(directive, UnprocessedEndDirective):
        ...
        # no exception as grammar prevents other directive kinds
        assert False, "could not classify generic directive"
...
def set_acc_parse_actions(grammar):
    ...
    grammar.acc_generic_mapping_clause.setParseAction(
        UnprocessedGenericMappingClause)
    grammar.acc_noarg_clause.setParseAction(UnprocessedNoArgumentClause)
    grammar.acc_artificial_clause_routine.setParseAction(
        TTAccArtificialClauseRoutine)
    grammar.acc_artificial_clause_cache.setParseAction(
        TTAccArtificialClauseCache)
    #
