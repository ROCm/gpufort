...
class TTCufKernelDo(base.TTNode,directives.IComputeConstruct,directives.ILoopAnnotation):
    def _assign_fields(self,tokens):
...
class TTCufAllocate(base.TTNode): # TODO not specific to CUF
    ...
    def hip_f_str(self,bytes_per_element,array_qualifiers,indent="",vars_are_c_ptrs=False):
        ...
        for i,array in enumerate(self._vars):
            if vars_are_c_ptrs: 
                size = array.size(bytes_per_element[i],base.make_f_str) # total size in bytes
            else:
