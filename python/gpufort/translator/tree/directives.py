...
def _search_values_in_subtree(ttnode,search_filter,scope,min_rank=-1):
    ...
    for ttvalue in find_all_matching_exclude_directives_(ttnode.body,search_filter): # includes the identifiers of the function calls
        ...
        tag = ttvalue.f_str()
        ivar,found_in_scope = indexer.scope.search_scope_for_variable(scope,tag)
...
def _flag_tensors(ttcontainer,scope):
    ...
    for value in base.find_all(ttcontainer.body,fortran.IValue):
        if type(value._value) is fortran.TTFunctionCallOrTensorAccess:
            _, discovered = indexer.scope.search_scope_for_variable(scope,value.f_str())
            if discovered:
...
class TTDo(base.TTContainer):
    ...
    def hip_thread_index_c_str(self):
        ivar = self.loop_var()
        begin    = base.make_c_str(self._begin._rhs) # array indexing is corrected in index macro
...
class TTLoopNest(base.TTContainer,IComputeConstruct):
    ...
    def __local_scalars_and_reduction_candidates(self,scope):
        ...
        initialized_scalars = [] 
        # depth first search
        assignments = base.find_all_matching(self.body[0],
                lambda node: type(node) in [fortran.TTAssignment,fortran.TTComplexAssignment,fortran.TTMatrixAssignment])
        for assignment in assignments:   
            ...
            if type(lvalue) is fortran.TTIdentifier: # could still be a matrix
                definition,found_in_scope = indexer.scope.search_scope_for_variable(scope,lvalue_name)
            ...
            # rhs scalars
            rhs_identifiers = base.find_all(assignment._rhs,fortran.TTIdentifier)
            for ttidentifier in rhs_identifiers:
                rvalue_name = ttidentifier.f_str().lower()
                definition,found_in_scope = indexer.scope.search_scope_for_variable(scope,rvalue_name)
                if (not found_in_scope or definition["rank"] == 0) and\
    ...
    def c_str(self):
        ...
        for expr in base.find_all(self.body[0],fortran.TTAssignment):
            for value in base.find_all_matching(expr,lambda x: isinstance(x,fortran.IValue)):
                if type(value._value) in [fortran.TTDerivedTypeMember,fortran.TTIdentifier]:
        ...
        for kind,reduced_variables in self.gang_team_reductions(base.make_c_str).items():
        ...
        do_loops = base.find_all(self.body[0],TTDo)
        if num_outer_loops_to_map == 1 or (opts.loop_collapse_strategy == "grid" and num_outer_loops_to_map <= 3):
            if num_outer_loops_to_map > 3:
                util.logging.log_warn("loop collapse strategy grid chosen with nested loops > 3")
            num_outer_loops_to_map = min(3,num_outer_loops_to_map)
