    ...
    def is_real(self,kind=None):
        ...
        if is_real and kind != None:
            if kind in fortran_type_2_bytes_map["real"]:
                ...
                default_real_bytes = fortran_type_2_bytes_map["real"][""].strip()
                kind_bytes         = fortran_type_2_bytes_map["real"][kind].strip()
                if len(parts)==1:   # no suffix
                    cond = False
                    cond = cond or (has_exponent_d and kind_bytes=="8")
                    cond = cond or (has_exponent_e and kind_bytes==default_real_bytes)
                    cond = cond or (not has_exponent and kind_bytes==default_real_bytes)
                    return cond
                elif len(parts)==2: # suffix
                    suffix = parts[1]
                    if suffix in fortran_type_2_bytes_map["real"]:
                        suffix_bytes = fortran_type_2_bytes_map["real"][suffix].strip()
    ...
    def is_integer(self,kind=None):
        ...
        if is_integer and kind != None:
            if kind in opts.fortran_type_2_bytes_map["integer"]:
                ...
                has_exponent_d     = "d" in self._value
                has_exponent       = has_exponent_e or has_exponent_d
                default_integer_bytes = opts.fortran_type_2_bytes_map["integer"][""].strip()
                kind_bytes            = opts.fortran_type_2_bytes_map["integer"][kind].strip()
                if len(parts)==1:   # no suffix
                    return kind_bytes == default_integer_bytes
                elif len(parts)==2: # suffix
                    suffix = parts[1]
                    if suffix in opts.fortran_type_2_bytes_map["integer"]:
                        suffix_bytes = opts.fortran_type_2_bytes_map["integer"][suffix].strip()
                        return kind_bytes == suffix_bytes
                    else:
...
class TTFunctionCallOrTensorAccess(base.TTNode):
    ...
    def has_matrix_range_args(self):
        ...
        """
        return not base.find_first(self._args,searched_type=TTMatrixRange) is None
    ...
    def c_str(self):
        ...
        if (not opts.fortran_style_tensor_access) and self.is_tensor():
            return "{0}[_idx_{0}({1})]".format(name,",".join([base.make_c_str(s) for s in self._args])) # Fortran identifiers cannot start with "_"
        else:
            return "{}({})".format(name,",".join([base.make_c_str(s) for s in self._args]))
    def f_str(self):
        name = base.make_f_str(self._name)
        return "{0}({1})".format(name,",".join([base.make_f_str(s) for s in self._args]))
class IValue:
...
class TTSizeInquiry(base.TTNode):
    ...
    def c_str(self):
        ...
        if type(self._dim) is TTNumber:
            return base.make_c_str(self._ref) + "_n" + base.make_c_str(self._dim) 
        else:
            prefix = "size"
...
class TTLboundInquiry(base.TTNode):
    ...
    def c_str(self):
        ...
        """
        if type(self._dim) is TTNumber:
            return base.make_c_str(self._ref) + "_lb" + base.make_c_str(self._dim) 
        else:
            prefix = "lbound"
            return "/* "+prefix+"("+base.make_f_str(self._ref)+") */"
    def f_str(self):
...
class TTConvertToExtractReal (base.TTNode):
    ...
    def c_str(self):
        c_type = conv.convert_to_c_type("real",self._kind).replace(" ","_") # TODO check if his anything else than double or float
        return "make_{1}({0})".format(base.make_c_str(self._ref),c_type) # rely on C++ compiler to make the correct type conversion
    def f_str(self):
...
class TTConvertToDouble(base.TTNode):
    def _assign_fields(self,tokens):
        self._ref, self._kind = tokens
    def c_str(self):
        return "make_double({0})".format(base.make_c_str(self._ref)) # rely on C++ compiler to make the correct type conversion
    def f_str(self):
        return "DBLE({0})".format(base.make_f_str(self._ref)) # rely on C++ compiler to make the correct type conversion
class TTConvertToComplex(base.TTNode):
    def _assign_fields(self,tokens):
        self._x, self._y, self._kind = tokens
    def c_str(self):
        c_type = conv.convert_to_c_type("complex",self._kind,default=None,float_complex="float_complex",double_complex="double_complex")
        return "make_{2}({0}, {1})".format(base.make_c_str(self._x),base.make_c_str(self._y),c_type)
    def f_str(self):
        result ="CMPLX({0},{1}".format(base.make_f_str(self._x),base.make_f_str(self._y))
        if not self._kind is None:
            result += ",kind={0}".format(base.make_f_str(self._kind))
        return result + ")"
class TTConvertToDoubleComplex(base.TTNode):
    ...
    def c_str(self):
        ...
        return "make_{2}({0}, {1})".format(base.make_c_str(self._x),base.make_c_str(self._y),c_type)
    def f_str(self):
        result ="DCMPLX({0},{1}".format(base.make_f_str(self._x),base.make_f_str(self._y))
        return result + ")"
...
class TTDerivedTypeMember(base.TTNode):
    ...
    def last_element_has_matrix_range_args(self):
        ...
        if type(current) is TTFunctionCallOrTensorAccess:
            ...
            return False
    def c_str(self):
        return base.make_c_str(self._type) + "." + base.make_c_str(self._element)
    def f_str(self):
        return base.make_f_str(self._type) + "%" + base.make_f_str(self._element)
...
class TTAssignment(base.TTNode):
    ...
    def f_str(self,unpack=False):
        ...
        if unpack:
           ...
           return self._lhs.f_str() + "=" + flatten_arithmetic_expression(self._rhs,converter=base.make_f_str) + ";\n"
...
class TTComplexAssignment(base.TTNode):
    ...
    def c_str(self):
        ...
        """
        result = ""
        result += "{}.x = {};\n".format(base.make_c_str(self._lhs),base.make_c_str(self._rhs._real))
        result += "{}.y = {};\n".format(base.make_c_str(self._lhs),base.make_c_str(self._rhs._imag))
        return result
class TTMatrixAssignment(base.TTNode):
    ...
    def c_str(self):
        ...
        for expression in self._rhs: 
            result += base.make_c_str(self._lhs) + argument + "=" + flatten_arithmetic_expression(expression) + ";\n"
        return result
...
class TTBounds(base.TTNode):
    ...
    def bound_variable_names(self,variable_name,converter=base.make_c_str):
        ...
        return result
    def bound_variable_assignments(self,variable_name,converter=base.make_c_str):
        """
    ...
    def index_str(self,variable_name,use_place_holders=False,macro_args=[]):
        ...
        bounds          = self.specified_bounds(base.make_c_str) if bounds_specified else self._bounds
        lower_bounds     = self.specified_lower_bounds(base.make_c_str) if bounds_specified else ["1"]*len(self._bounds) # TODO
        if bounds_specified:
        ...
        if len(bounds):
            ...
            return None
    def index_macro_str(self,variable_name,converter=base.make_c_str,use_place_holders=False):
        """
        ...
        """
        bounds_specified = not self.has_unspecified_bounds()
        bounds = list(self.specified_bounds(base.make_c_str)) if bounds_specified else self._bounds
        macro_args = [chr(ord('a')+i) for i in range(0,len(bounds))]
    ...
    def index_macro_f_str(self,variable_name,use_place_holders=False):
        return self.index_macro_str(variable_name,base.make_f_str,use_place_holders)
    def index_macro_c_str(self,variable_name,use_place_holders=False):
        return self.index_macro_str(variable_name,base.make_c_str,use_place_holders)
    def c_str(self):
        """
        ...
        return "".join(["[{0}]".format(base.make_c_str(el)) for el in self._bounds])
    def f_str(self):
        """
        Returns '(<bound1.f_str()>,<bound2.f_str()>,...,<bound(n).f_str()>)' for all n sizes corresponding to specified bounds
        """
        return "({0})".format(",".join(base.make_f_str(el) for el in self._bounds)) 
...
class TTDeclaration(base.TTNode,Attributed):
    ...
    def datatype_f_str(self):
        ...
        if self.type.lower() == "type":
            ...
            if self.kind != None:
                ...
                return opts.character_format.format(type=f_type,len="").lstrip()[0:len(f_type)]
        else:
            if self.kind != None:
                return opts.basic_type_format.format(type=f_type,kind=self.kind)
            else:
                return opts.basic_type_format.format(type=f_type,kind="").lstrip()[0:len(f_type)]
    def f_str(self,extra_ignore_list=[],include_bounds=True,include_rhs=True):
        ...
        qualifiers_str += ", ".join([base.make_f_str(q) for q in self.qualifiers])
        result = "{0}{1} :: ".format(self.datatype_f_str(),qualifiers_str)
        ignored_vars = 0
        for ttdeclaredvariable in self._rhs:
            if not ttdeclaredvariable.name(base.make_f_str).lower() in ignore_list:
                ignored_vars += 1
...
grammar.complex_arithmetic_expression.setParseAction(TTComplexArithmeticExpression)
