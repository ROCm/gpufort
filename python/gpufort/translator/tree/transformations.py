...
class TTUnrolledArrayAssignment(TTInjectedStatement):
    ...
    def cstr(self):
        return base.to_cstr(self.lhs) + "=" + base.to_cstr(self.rhs) + ";\n"
...
class TTCIdentifier(TTInjectedNode):
    ...
    def __str__(self):
        return "TTCIdentifier(name:"+str(self._name)+")"
    __repr__ = __str__


class TTCFunctionCall(TTInjectedNode):
    ...
    def cstr(self):
        return self.name + "(" + ",".join([base.to_cstr(arg) for arg in self.args]) + ")"
...
class TTCElseIf(TTInjectedContainer):
    ...
    def child_nodes(self):
        if isinstance(self.condition, base.TTNode):
            yield self.condition
        yield from self.body

    def header_cstr(self):
        prefix = self.is_elseif+" " if self.is_elseif != None else ""
        return "{}if ({})".format(prefix, base.to_cstr(self.condition))
...
class TTCForLoop(TTInjectedContainer):
    ...
    def __init__(self,
        ...
        Args:
            index: _description_
            excl_ubound: _description_
            incl_lbound (optional): Inclusive lower bound. Defaults to None.
            step (optional): Step size. Defaults to None.
        """
        self._init()
        self.index = index
        self.incl_lbound = incl_lbound
    ...
    @property
    def first(self):
        if self.incl_lbound != None:
            return self.incl_lbound
        else:
            return tree.TTNumber(["1"])

    @property
    def last(self):
        return arithexpr.TTBinaryOpChain.subtract(
            self.excl_ubound,
            tree.TTNumber(["1"])
        )
...
for ({idx} = {ilb};
     {idx} < {eub}; {idx} += {step_as_str})""".format(
            ...
            ilb=base.to_cstr(
                self.incl_lbound) if self.incl_lbound != None else "0",
            eub=base.to_cstr(self.excl_ubound),
            step_as_str=base.to_cstr(self.step) if self.step != None else "1"
        )


class TTCAssignment(TTInjectedStatement):
    """Emits an assignment."""
...
class TTCVarDecl(TTInjectedStatement):
    ...
    def cstr(self):
        if self.rhs != None:
            template = "{modifier}{type} {var} = {rhs};"
        else:
            template = "{modifier}{type} {var};"
        return template.format(
            modifier=self.modifier+" " if self.modifier != None else "",
            type=self.type,
            var=base.to_cstr(self.var_expr),
            rhs=base.to_cstr(self.rhs)
        )
...
class TTCReductionBlockwiseAggregation(TTInjectedContainer):
    ...
    def __init__(self,
        ...
        #  TTCReductionOpApplication(
        #    buffer_read_expr_as_cstr, # notice that buffer and var expr are swapped
        #    var_expr_as_cstr,
        #    c_type_as_cstr,
        #    fortran_op_as_cstr,
        #  )
...
class TTCGpufortArrayPtrDecl(TTCVarDecl):
    ...
    def __init__(self,
        ...
        TTCVarDecl.__init__(self,
                            render_gpufort_array_descr_type(
                                symbol_info.rank, element_type),
                            var_name
                            )
