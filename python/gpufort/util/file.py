...
def prettify_c_code(cCode, style):
    ...
    """
    command = "printf \"{0}\" | clang-format -style={1}".format(
        cCode.replace("%", "%%"), style)
...
def prettify_c_file(cPath, style):
    ...
    /opt/rocm-<suffix>/hcc/bin/clang-format"
    """
    command = "clang-format -i -style={1} {0}".format(cPath,
                                                      style) # writes inplace
...
def prettify_f_file(fPath):
    ...
    Requires fprettify
    """
    subprocess.check_call(["fprettify", "-l 1000", fPath],
                          stdout=subprocess.DEVNULL)
    subprocess.check_call(
        r"sed -i 's,\s*\([<>]\)\s*[<>]\s*[<>]\s*,\1\1\1,g' {0}".format(fPath),
        shell=True,
        stdout=subprocess.DEVNULL)
    #return subprocess.check_output(command,shell=True).decode('ascii')
    #pass
...
def read_c_file_without_comments(file_path, unifdef_args=""):
    ...
    """
    command = "gcc -w -fmax-errors=100 -fpreprocessed -dD -E {0}".format(
        file_path)
    if len(unifdef_args):
