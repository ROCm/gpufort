...
def log_debug(prefix,func_name,raw_msg,debug_level=1):
    ...
    if opts.log_filter == None or re.search(opts.log_filter,msg):
        ...
        if opts.verbose and __LOG_LEVEL_AS_INT <= getattr(logging,"DEBUG")-debug_level+1:
...
def log_leave_function(prefix,func_name,retvals={}):
    ...
    if len(retvals):
        addition = " [return values: "+ ", ".join(a+"="+str(retvals[a]) for a in retvals.keys())+"]"
    else:
        addition = "" 
    log_debug(prefix,func_name,"return"+addition)

def log_entry_and_exit(prefix,print_args=False,print_kwargs=False,print_retvals=False):
    ...
    def inner1(func):
        def inner2(*args,**kwargs):
            ...
            if print_args and len(*args):
                enter_args += [{ "#{}".format(i) : a} for i,a in enumerate(args)]
            ...
            if print_retvals and len(retvals):
                leave_args += [{ "#{}".format(i) : a} for i,a in enumerate(args)]
