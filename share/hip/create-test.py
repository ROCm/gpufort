...
def parseCommandLineArguments():
    ...
    parser = argparse.ArgumentParser(description='Create (Fortran) unit tests from a HIP C++ kernel source that has been generated with hipfortify.py.')
...
"""
arrayHostDeclTemplate  = """{indent}{type},allocatable :: {var}_h({dimensions})
""" 
arrayHostInitTemplate  = """{indent}allocate({var}_h({sizes})); {indent}{var}_h = {val}
""" 
arrayHostFreeTemplate  = """{indent}deallocate({var}_h)
"""
arrayDevDeclTemplate  = """{indent}{type},pointer :: {var}({dimensions})
"""
arrayDevInitTemplate = """{indent}CALL hipCheck(hipMalloc({var}, source={var}_h));
"""
arrayDevFreeTemplate  = """{indent}CALL hipCheck(hipFree({var}));
"""
kernelLaunchTemplate  = """{indent}CALL {name}(0,nullptr,{argNames});
"""

testTemplate="""
  function test_{name}({argNames}) result(error_code)
{includes}
{declarations}
{host_buffers}
  !
...
{init}
  !
  call {name}_cpu({argNames})

  ! copy result to host buffers 1

  call {name}_auto({argNames})

  ! copy result to host buffers 2

  ! compare host buffers 1 vs 2
